/**
 * ============================================================
 * email-ui.js  —  v2.0 (PER-PAGE TOGGLES)
 * Drop-in Email Notification Widget — Works on ANY HTML page
 *
 * WHAT CHANGED vs v1:
 *   The widget now renders a second toggle section showing all
 *   6 page-specific email toggles so the user can choose which
 *   pages send email reports. Each toggle calls
 *   EmailDB.setPagePref(uid, pageKey, enabled) individually.
 *
...
const PAGE_DEFS = [
    { key: 'advance_payment', label: 'অগ্রিম পরিশোধ',   icon: '💵' },
    { key: 'business_stats',  label: 'ব্যবসা পরিসংখ্যান', icon: '📊' },
    { key: 'donation',        label: 'অনুদান',            icon: '🤝' },
    { key: 'help',            label: 'সহায়তা',            icon: '📋' },
    { key: 'office_issue',    label: 'সমস্যা ও সমাধান',   icon: '⚠️' },
    { key: 'premium_submit',  label: 'প্রিমিয়াম জমা',    icon: '🏦' },
];
...
#email-notif-widget { display: block; width: 100%; }
.enotif-card h3 {
    margin: 0 0 4px; font-size: 1rem; font-weight: 700;
    color: var(--text, #111); display: flex; align-items: center; gap: 8px;
}
.enotif-card p.enotif-sub {
    margin: 0 0 16px; font-size: .82rem; color: var(--text-muted, #6b7280);
}
.enotif-badge {
...
.enotif-section-label {
    font-size: .78rem; font-weight: 700; color: var(--text-muted,#6b7280);
...
.enotif-label {
    font-size: .88rem; color: var(--text, #374151); display: flex; align-items: center; gap: 6px;
}
.enotif-switch {
...
.enotif-divider { border: none; border-top: 1px solid var(--border,#e5e7eb); margin: 14px 0; }
.enotif-btn {
...
.enotif-btn-primary { background: #251577; color: #fff; }
.enotif-btn-ghost {
    background: #dc3545; color: #fff; border: 1.5px solid #dc3545; margin-left: 8px;
}
.enotif-msg {
    margin-top: 12px; padding: 8px 12px; border-radius: 8px; font-size: .83rem; display: none;
}
...
function _render(container, { title, accent, sub, isSubscribed, email, prefs }) {
    ...
    const pageToggles = PAGE_DEFS.map(p =>
        _makeToggle(`enotif-page-${p.key}`, p.label, p.icon, pagePrefs[p.key] !== false)
    ).join('');
...
<div class="enotif-card">
    ...
    <div class="enotif-section-label">📅 ডাইজেস্ট সময়</div>
    ...
    <div class="enotif-section-label">📌 পেজ ভিত্তিক ইমেইল</div>
    ${pageToggles}
    ...
    <div class="enotif-toggle-row" style="align-items:flex-end">
        <div style="flex:1;margin-right:10px">
            ...
            <input
                ...
                style="width:100%;padding:7px 10px;border-radius:7px;
                       border:1.5px solid var(--border,#e5e7eb);font-size:.88rem;
                       box-sizing:border-box;background:var(--input-bg,#f9fafb);
                       color:var(--text,#111);"
            >
        </div>
    </div>
...
function _wireEvents(container, uid, currentEmail) {
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', async () => {
            ...
            // Collect per-page preferences
            const pages = {};
            PAGE_DEFS.forEach(p => {
                const el = container.querySelector(`#enotif-page-${p.key}`);
            ...
            const prefs = {
                ...
                tasks:   true,  // legacy key kept for compatibility
                pages:   pages
            };
            ...
            try {
                ...
                _showMsg('✅ সাবস্ক্রিপশন সেভ হয়েছে! আপনি ইমেইল পাবেন।', 'ok');
